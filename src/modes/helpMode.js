...
export async function initHelp(options) {
  ...
  console.log("");
  console.log(`\t${name} [flags] [project-path]`);
  console.log("");
  console.log("\t\tSetup or configure a project at a specified path (interactively)");
  console.log("");
  console.log("Usage:");
  console.log("");
  console.log(`\t${name} [mode] [...flags] [project-path]`);
  console.log("");
  console.log(" Modes:");
  const modeDescRec = {
    "--help / -h": "Display help",
    "--status":
      "Request the current status from the manager process and display it",
    "--logs":
      "Print the latest logs and stream them continuously as they are generated",
    "--start / --restart":
      "Start or restart the instance manager process and display its status",
    ...
    "--schedule":
      "Schedules an asyncronous execution of the deployment (in the background)",
    "--schedule <hash>": "Schedules the deployment at a specific commit",
    "--schedule <ref>":
      "Schedules the deployment at a specific reference or branch",
    "--deploy":
      "Execute the deployment steps syncronously at the current project version",
    "--deploy <hash>": "Execute the deployment at a specific commit",
    "--deploy <rev>":
      "Execute a deployment pipeline at a specific branch reference",
    "--manager":
      "Run this program to manage the project instance synchronously",
  };
  ...
  const flagDescs = [
    ...
    [
      "--dry-run / --dry",
      "Simulate execution by not writing files, causing no side-effects",
    ],
    [
      "--sync / --wait",
      "Execute the deployment pipelines syncronously when in schedule mode",
    ],
  ];
  const pad =
